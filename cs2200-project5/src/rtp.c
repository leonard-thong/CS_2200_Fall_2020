...
int checksum(char *buffer, int length) {
     ...
     *  to implement this function.  For simplicity, simply xor the ascii
     *  values of all the characters in the buffer, and return that value.
     */
    int xor = buffer[0];
...
packet_t *packetize(char *buffer, int length, int *count) {
    ...
    *count = length / MAX_PAYLOAD_LENGTH;
    ...
    packets = (packet_t*) malloc((size_t) *count * sizeof(packet_t));
    for (int i = 0; i < *count; i++) {
        packet_t* packet = &packets[i];
        if (i == *count - 1) {
            packet -> type = LAST_DATA;
            packet -> payload_length = length % MAX_PAYLOAD_LENGTH;
        } else {
            packet -> type = DATA;
            packet -> payload_length = MAX_PAYLOAD_LENGTH;
        }
        memcpy(packet -> payload, buffer + i * MAX_PAYLOAD_LENGTH, MAX_PAYLOAD_LENGTH);
            ...
            */
            if (packet.type == DATA || packet.type == LAST_DATA) {
                ...
                if (packet.checksum == checksum(packet.payload, packet.payload_length)) {
                    response -> type = ACK;
                    unsigned int old_length = buffer_length;
                    buffer_length = buffer_length + packet.payload_length;
                    buffer = (char*)realloc(buffer, buffer_length * sizeof(char));
                    memcpy(buffer + old_length, packet.payload, packet.payload_length);
